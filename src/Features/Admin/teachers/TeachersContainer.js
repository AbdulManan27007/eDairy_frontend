...
export const TeachersContainer = () => {
  ...
  useEffect(() => {
    ...
    if (term.trim() === "") {
      ...
      const teachersmap = data?.map((item, index) => ({
        id: index + 1,
        ...item,
      }));

      if (teachersmap && teachersmap.length) {
        const teachers = teachersmap.filter(
          (item) => item.access.role == "teacher"
        );
      ...
      if (filteredExaminations && filteredExaminations?.length) {
        const teachers = filteredExaminations.filter(
          (item) => item.access.role == "teacher"
        );
        setFilteredData(teachers || []);
      }
