...
import { UPDATE_BY_ID } from "../../../services/ClassSections";
...
export const Attendance = () => {
  ...
  const { user, students, setStudents, selectedClass, selectedSection } =
    context;
  ...
  useEffect(() => {
    if (selectedSection?.id) {
      GET_ALL_students({ section: selectedSection.id })
        .then((resp) => {
  ...
  const Attndnce = (data) => {
    UPDATE_BY_ID({ id: selectedSection.id, attendance: data })
