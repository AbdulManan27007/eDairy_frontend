...
const Chats = (props) => {
  ...
  const { user, userInfo, setDialog, chosenDialog, dialogs } = props;
  useEffect(() => {
    try {
  ...
  useEffect(() => {
    if (location.state.id == userInfo._id && !chosenDialog) {
      setDialog(userInfo);
    }
  }, []);

  const retrieveChat = () => {
    navigate(`/dashboard/${user.role}/home`, { state: { id: userInfo._id } });
